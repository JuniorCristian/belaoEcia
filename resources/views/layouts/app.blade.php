...
@stack('scripts')
<script>
    function gerenciarFase() {
        $('.gerenciar-fases').click(function () {
            const gerenciar_Fase = Swal.mixin({
                customClass: {
            ...
            gerenciar_Fase.fire({
                ...
                html: '<div class="row">' +
                    ...
                    '         <div class="form-group">' +
                    @for($i = 0; $i < 15; $i++)
                        '             <img src="{{env('APP_URL')}}/storage/examples/example1-300x300.jpg" class="img-thumbnail rounded float-left" style="width:150px!important" alt="...">' +
                    @endfor
                        '         </div>' +
                    '     </div>' +
                    '</div>'
